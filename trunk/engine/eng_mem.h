...
#define MAX_ALIGNMENT	sizeof(unsigned long)
#define alignment_of(_type)					\
	((sizeof (_type) <= MAX_ALIGNMENT) ? sizeof (_type) : MAX_ALIGNMENT)
#define align_ptr(_ptr,_alignment)			\
	(((char*)(_ptr)) + ((_alignment) - (((((unsigned long)(_ptr)) - 1) % (_alignment)) + 1)))
#define align_offset_ex(_offset,_alignment)	\
	((_offset) + ((_alignment) - (((((unsigned long)(_offset)) - 1) % (_alignment)) + 1)))
#define align_offset(_offset, _alignment)	\
	((_alignment) == 0) ? (_offset) : align_offset_ex((_offset), (_alignment))
